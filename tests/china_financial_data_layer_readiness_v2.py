#!/usr/bin/env python3
"""China Financial Draft Data-Layer Readiness V2.

Normative baseline: current internal China Financial Draft.
Historical/reference only: untouched V0.9.1.1 manuscript.

READY scope is strictly DATA_COLLECTION_AND_MEASUREMENT_LAYER. This audit does
not declare downstream Permission logic, the trading model, a scheduled runtime
Store/SLA, or the repository as a whole to be Production.

V2 also formalizes source-health degradation for supporting fiscal cash-clock
context: temporary government-bond source outages become explicit UNKNOWN /
Data-Confidence warnings and do not fabricate zero or relax any core financial-
condition gate.
"""
...
def main() -> int:
    ...
    capabilities = {
        ...
        "local_gov_cash_clock_context": file_gate([
        ...
        "central_gov_cash_clock_ready_context": file_gate([
            "collectors/china_financial/central_gov_bond_cash_clock_v5_context.py",
            ".github/workflows/china-financial-central-gov-cash-clock-v5-ready-test.yml",
        ]),
        "unified_ready_gate": file_gate([".github/workflows/china-financial-daily.yml"]),
    }
    ...
    core_measurement_pass = all(
        x["status"] == "PASS" for x in (funding, risk, slow, fiscal, policy_monthly)
    )
    contract_ready = (
    ...
    report = {
        "audit": "CHINA_FINANCIAL_DRAFT_DATA_LAYER_READINESS_V2",
        ...
        "metadata_and_runtime_closure": metadata_runtime,
        "persisted_series_count": len(present),
        "operating_policy": {
            "default": "DAILY_INCREMENTAL",
            "high_frequency": "LATEST_ELIGIBLE_SESSION_OR_RECENT_WINDOW",
