...
final,raw,html=get(LIST); p=LinkParser(); p.feed(html)
items=[]
for href,title in p.links:
    if not href or not title: continue
    if '2026' not in title: continue
    if '财政收支情况' not in title: continue
    url=urljoin(final,href)
    try:
        f,r,h=get(url); text=clean(h)
        contexts=[]
        for needle in ('全国一般公共预算收入','全国一般公共预算支出','全国政府性基金预算收入','全国政府性基金预算支出'):
            m=text.find(needle)
            contexts.append({'needle':needle,'context':text[max(0,m-120):m+360] if m>=0 else None})
        meta=re.search(r'<meta\s+name=["\']createDate["\']\s+content=["\']([^"\']+)',h,re.I)
        items.append({'title':title,'url':f,'sha256':hashlib.sha256(r).hexdigest(),'createDate':meta.group(1) if meta else None,'contexts':contexts})
    except Exception as e:
        items.append({'title':title,'url':url,'error':repr(e)})
print(json.dumps({'list_url':final,'list_sha256':hashlib.sha256(raw).hexdigest(),'count':len(items),'items':items},ensure_ascii=False,indent=2))
